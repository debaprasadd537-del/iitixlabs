...
const C = {
  navy: "#0a1628", navyMid: "#0d1f3c", navyLight: "#132447",
  cyan: "#00b4d8", cyanSoft: "#90e0ef", cyanGlow: "rgba(0,180,216,0.12)",
  white: "#f8fafc", gray50: "#f1f5f9", gray100: "#e2e8f0",
  gray300: "#94a3b8", gray500: "#64748b", gray700: "#334155",
  accent: "#38bdf8", accentAlt: "#0ea5e9",
...
const s = {
  ...
  badge: (color = C.cyan) => ({ display: "inline-block", padding: "clamp(2px, 0.5vw, 2px) clamp(8px, 1.5vw, 10px)", borderRadius: 20, fontSize: "clamp(10px, 1.5vw, 11px)", fontWeight: 600, background: `${color}22`, color, border: `1px solid ${color}44`, letterSpacing: "0.04em" }),
  tag: (color = C.cyan) => ({ display: "inline-block", padding: "clamp(2px, 0.5vw, 3px) clamp(8px, 1.5vw, 10px)", borderRadius: 6, fontSize: "clamp(11px, 1.5vw, 12px)", background: `${color}18`, color, border: `1px solid ${color}30` }),
};
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${C.navy} 0%, #060e1f 100%)`, display: "flex", flexDirection: "column", fontFamily: "'Segoe UI', system-ui, sans-serif", overflow: "hidden", position: "relative" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "clamp(16px, 3vw, 20px) clamp(20px, 5vw, 48px)", position: "relative", zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div><div style={{ fontWeight: 700, fontSize: 16, letterSpacing: "0.12em", color: C.white }}>IITIX LABS</div><div style={{ fontSize: 9, letterSpacing: "0.18em", color: C.gray300, textTransform: "uppercase" }}>Operational Intelligence</div></div>
        </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "clamp(40px, 8vw, 60px) clamp(16px, 4vw, 24px)", position: "relative", zIndex: 10 }}>
        <div style={{ ...s.badge(), marginBottom: 24 }}>PREPARATION COMMAND CENTER v2.5</div>
        <h1 style={{ fontSize: "clamp(28px, 5vw, 60px)", fontWeight: 200, letterSpacing: "-0.03em", color: C.white, lineHeight: 1.1, margin: "0 0 16px", maxWidth: 720 }}>
          Operational Intelligence<br /><span style={{ fontWeight: 600, color: C.cyan }}>for Serious Aspirants</span>
        </h1>
        <p style={{ fontSize: "clamp(14px, 2vw, 16px)", color: C.gray300, maxWidth: 540, lineHeight: 1.7, margin: "0 0 44px" }}>A disciplined command center for JEE and NEET preparation.</p>
...
function LoginPage({ onLogin, onSignup, onBack }) {
  ...
  const handle = () => {
    ...
    if (u) { store.set("currentUser", u.id); onLogin(u); } else setErr("Invalid credentials.");
  };
  return (
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', system-ui, sans-serif", padding: "clamp(16px, 4vw, 40px)" }}>
      <div style={{ width: "clamp(280px, 90%, 520px)", background: C.navyMid, border: `1px solid ${C.glassBorder}`, borderRadius: 16, padding: "clamp(24px, 5vw, 40px)" }}>
        ...
        <div style={{ ...s.badge(), marginBottom: 16 }}>{cur.label}</div>
...
function DailyInitialization({ user, onComplete }) {
  const goals = [
    ...
    { key: "energyLevel", label: "Energy Level", question: "How is your energy level today?", options: ["Peak", "Good", "Moderate", "Low — light revision"] },
  ];
  const [step, setStep] = useState(0);
  const [data, setData] = useState({});
  const select = val => {
    ...
    setData(nd);
    if (step < goals.length - 1) setStep(step + 1);
    else onComplete(nd);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', system-ui, sans-serif", padding: "clamp(16px, 4vw, 40px)" }}>
      <div style={{ width: "clamp(280px, 90%, 520px)", background: C.navyMid, border: `1px solid ${C.glassBorder}`, borderRadius: 16, padding: "clamp(24px, 5vw, 40px)" }}>
        <div style={{ marginBottom: 8 }}><span style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: C.gray300 }}>Daily Setup — Step {step + 1} of {goals.length}</span></div>
...
// ─── TIMER CONTROLLER (ROOT LEVEL HOOK) ──────────────────────────────────
function useTimerController() {
  const [pomodoroState, setPomodoroState] = useState(() => {
    const saved = store.get("timerState", null);
    const defaults = { mode: "Deep Work", phase: "work", isRunning: false, startTimestamp: null, totalDuration: 50 * 60 };
    if (saved && saved.pomodoroState) {
      const pom = saved.pomodoroState;
      if (pom.isRunning && pom.startTimestamp) {
        const elapsedSec = Math.floor((Date.now() - pom.startTimestamp) / 1000);
        const remaining = Math.max(0, pom.totalDuration - elapsedSec);
        if (remaining > 0) return { ...pom, isRunning: true };
        const newPhase = pom.phase === "work" ? "rest" : "work";
        const cfg = POMODORO_PRESETS[pom.mode];
        return { ...pom, phase: newPhase, isRunning: false, startTimestamp: null, totalDuration: newPhase === "work" ? cfg.work * 60 : cfg.rest * 60 };
      }
      return { ...pom, isRunning: false };
    }
    return defaults;
  });

  const [stopwatchState, setStopwatchState] = useState(() => {
    const saved = store.get("stopwatchState", null);
    const defaults = { isRunning: false, startTimestamp: null, elapsedMillis: 0, sessionLabel: "Physics", sessions: store.get("sessions", []) };
    if (saved) {
      if (saved.isRunning && saved.startTimestamp) {
        const drift = Date.now() - saved.startTimestamp;
        return { ...saved, elapsedMillis: saved.elapsedMillis + drift, isRunning: true, startTimestamp: saved.startTimestamp };
      }
      return { ...saved, isRunning: false, sessions: saved.sessions || defaults.sessions };
    }
    return defaults;
  });
  ...
  useEffect(() => { store.set("timerState", { pomodoroState }); }, [pomodoroState]);
  useEffect(() => { store.set("stopwatchState", { ...stopwatchState, sessions: stopwatchState.sessions }); }, [stopwatchState]);

  // Pomodoro tick — uses timestamp, never drifts
  useEffect(() => {
    if (!pomodoroState.isRunning || !pomodoroState.startTimestamp) {
    ...
    pomodoroInterval.current = setInterval(() => {
      setPomodoroState(prev => {
        ...
        const elapsedSec = Math.floor((Date.now() - prev.startTimestamp) / 1000);
        const remaining = Math.max(0, prev.totalDuration - elapsedSec);
        if (remaining <= 0) {
          const newPhase = prev.phase === "work" ? "rest" : "work";
          const cfg = POMODORO_PRESETS[prev.mode];
          const newDuration = newPhase === "work" ? cfg.work * 60 : cfg.rest * 60;
          return { ...prev, phase: newPhase, isRunning: false, startTimestamp: null, totalDuration: newDuration };
        }
        return prev; // no state change needed — display computes from timestamp
      });
    }, 1000);
  ...
  // Force re-render every second for display while running
  const [, setTick] = useState(0);
  useEffect(() => {
    if (!pomodoroState.isRunning && !stopwatchState.isRunning) return;
    const id = setInterval(() => setTick(t => t + 1), 1000);
    return () => clearInterval(id);
  }, [pomodoroState.isRunning, stopwatchState.isRunning]);

  // Stopwatch tick
  useEffect(() => {
    if (!stopwatchState.isRunning || !stopwatchState.startTimestamp) {
      if (stopwatchInterval.current) { clearInterval(stopwatchInterval.current); stopwatchInterval.current = null; }
      return;
    }
    stopwatchInterval.current = setInterval(() => {
      setStopwatchState(prev => {
        if (!prev.isRunning || !prev.startTimestamp) return prev;
        const drift = Date.now() - prev.startTimestamp;
        return { ...prev, elapsedMillis: prev.elapsedMillis + drift, startTimestamp: Date.now() };
      });
    }, 100);
    return () => { if (stopwatchInterval.current) clearInterval(stopwatchInterval.current); };
  }, [stopwatchState.isRunning, stopwatchState.startTimestamp]);

  return { pomodoroState, setPomodoroState, stopwatchState, setStopwatchState };
}
...
function PomodoroTimer({ pomodoroState, setPomodoroState }) {
  const handleModeChange = (newMode) => {
    ...
    setPomodoroState({ mode: newMode, phase: "work", isRunning: false, startTimestamp: null, totalDuration: cfg.work * 60 });
  };
  const handleStart = () => {
    if (!pomodoroState.isRunning) setPomodoroState(s => ({ ...s, isRunning: true, startTimestamp: Date.now() }));
    else setPomodoroState(s => ({ ...s, isRunning: false, startTimestamp: null }));
  };
  const handleReset = () => {
    const cfg = POMODORO_PRESETS[pomodoroState.mode];
    setPomodoroState({ mode: pomodoroState.mode, phase: "work", isRunning: false, startTimestamp: null, totalDuration: cfg.work * 60 });
  };
  const elapsedSec = pomodoroState.isRunning && pomodoroState.startTimestamp ? Math.floor((Date.now() - pomodoroState.startTimestamp) / 1000) : 0;
  const remaining = Math.max(0, pomodoroState.totalDuration - elapsedSec);
  const mins = Math.floor(remaining / 60);
  const secs = remaining % 60;
  const progress = pomodoroState.totalDuration > 0 ? ((pomodoroState.totalDuration - remaining) / pomodoroState.totalDuration) * 100 : 0;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "clamp(16px, 3vw, 24px)" }}>
      ...
      <div style={{ ...s.card, background: `linear-gradient(135deg, rgba(0,180,216,0.08), rgba(13,31,60,0.5))`, border: `1px solid ${preset.color}40`, textAlign: "center", padding: "clamp(32px, 6vw, 40px)" }}>
        ...
        <div style={{ display: "flex", gap: "clamp(8px, 2vw, 12px)", justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={handleStart} style={{ ...s.btn, background: preset.color, padding: "clamp(8px, 2vw, 10px) clamp(20px, 4vw, 28px)" }}>{pomodoroState.isRunning ? "Pause" : "Start"}</button>
          <button onClick={handleReset} style={{ ...s.btnOutline, borderColor: preset.color, color: preset.color }}>Reset</button>
        </div>
      </div>
    </div>
  );
...
function Stopwatch({ stopwatchState, setStopwatchState }) {
  const handleStart = () => {
    if (!stopwatchState.isRunning) setStopwatchState(s => ({ ...s, isRunning: true, startTimestamp: Date.now() }));
    else setStopwatchState(s => ({ ...s, isRunning: false, startTimestamp: null }));
  };
  const handleReset = () => setStopwatchState(s => ({ ...s, elapsedMillis: 0, isRunning: false, startTimestamp: null }));
  const saveSession = () => {
    ...
    setStopwatchState(s => ({ ...s, sessions: updated, elapsedMillis: 0 }));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "clamp(16px, 3vw, 24px)" }}>
      ...
      <div style={{ ...s.card, background: "linear-gradient(135deg, rgba(10,22,40,0.8), rgba(13,31,60,0.6))", border: `1px solid ${C.cyan}30`, textAlign: "center", padding: "clamp(32px, 6vw, 48px)", boxShadow: "inset 0 2px 8px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 0, fontVariantNumeric: "tabular-nums", fontFamily: "'Courier New', monospace", flexWrap: "wrap" }}>
          {hours > 0 && <div style={{ fontSize: "clamp(36px, 8vw, 48px)", fontWeight: 300, color: C.cyan }}>{String(hours).padStart(2, "0")}</div>}
          {hours > 0 && <div style={{ fontSize: "clamp(36px, 8vw, 48px)", fontWeight: 300, color: C.gray500, margin: "0 2px" }}>:</div>}
          <div style={{ fontSize: "clamp(36px, 8vw, 48px)", fontWeight: 300, color: C.cyan }}>{String(mins).padStart(2, "0")}</div>
          <div style={{ fontSize: "clamp(36px, 8vw, 48px)", fontWeight: 300, color: C.gray500, margin: "0 2px" }}>:</div>
          <div style={{ fontSize: "clamp(36px, 8vw, 48px)", fontWeight: 300, color: C.cyan }}>{String(secs).padStart(2, "0")}</div>
          <div style={{ fontSize: "clamp(24px, 5vw, 32px)", fontWeight: 300, color: C.gray500, margin: "0 2px", alignSelf: "flex-end", marginBottom: 8 }}>.{ms}</div>
        </div>
        <div style={{ marginTop: "clamp(20px, 4vw, 28px)", display: "flex", gap: "clamp(8px, 2vw, 12px)", justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={handleStart} style={{ ...s.btn, background: C.cyan, padding: "clamp(8px, 2vw, 10px) clamp(20px, 4vw, 28px)" }}>{stopwatchState.isRunning ? "Pause" : "Start"}</button>
          <button onClick={handleReset} style={{ ...s.btnOutline, borderColor: C.cyan, color: C.cyan }}>Reset</button>
          {elapsed > 0 && <button onClick={saveSession} style={{ ...s.btn, background: C.success, padding: "clamp(8px, 2vw, 10px) clamp(16px, 3vw, 28px)" }}>Save</button>}
        </div>
      </div>
        ...
        <div style={s.card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "clamp(6px, 1.5vw, 8px)" }}>
            {stopwatchState.sessions.slice(-5).reverse().map((session, i) => {
              const h = Math.floor(session.duration / 3600000); const m = Math.floor((session.duration % 3600000) / 60000);
              return <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "clamp(6px, 1.5vw, 8px) 0", borderBottom: `1px solid ${C.glassBorder}`, fontSize: "clamp(12px, 1.8vw, 13px)" }}>
    ...
    <div>
      <div style={{ display: "flex", gap: "clamp(8px, 2vw, 12px)", marginBottom: "clamp(20px, 3vw, 28px)", borderBottom: `1px solid ${C.glassBorder}`, paddingBottom: "clamp(8px, 1.5vw, 12px)", flexWrap: "wrap", alignItems: "center" }}>
        {["pomodoro", "stopwatch"].map(tab => (
          ...
          }}>{tab === "pomodoro" ? "Pomodoro" : "Stopwatch"}</button>
        ))}
        {timerActive && <div style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: C.cyan, marginLeft: "auto", marginRight: 8, animation: "pulse 1s infinite" }} />}
      </div>
...
function AttendanceModule() {
  ...
  const markAttendance = () => {
    ...
    let status = "green";
    if (hour >= 7 && (hour < 8 || (hour === 8 && minute <= 30))) status = "yellow";
    else if (hour > 8 || (hour === 8 && minute > 30)) status = "red";
  ...
  const sc = (status) => status === "green" ? C.success : status === "yellow" ? C.warning : C.danger;
  const sl = (status) => status === "green" ? "On Time" : status === "yellow" ? "Late" : "Very Late";
  const monthStr = currentMonth.toISOString().split("T")[0].slice(0, 7);
  const monthRecords = Object.entries(attendance).filter(([d]) => d.startsWith(monthStr));
  const stats = { green: monthRecords.filter(([, r]) => r.status === "green").length, yellow: monthRecords.filter(([, r]) => r.status === "yellow").length, red: monthRecords.filter(([, r]) => r.status === "red").length };
  let streak = 0;
  const sortedDates = Object.keys(attendance).sort().reverse();
  if (attendance[today]) { streak = 1; for (let i = 0; i < sortedDates.length - 1; i++) { const c = new Date(sortedDates[i]), n = new Date(sortedDates[i + 1]); if (Math.floor((c - n) / 86400000) === 1) streak++; else break; } }
  ...
  return (
    <div>
      <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}><h1 style={{ ...s.h1, marginBottom: 4 }}>Attendance</h1><p style={{ ...s.muted }}>Track your daily presence</p></div>
      {!attendance[today] ? (
        <button onClick={markAttendance} style={{ ...s.btn, marginBottom: "clamp(20px, 3vw, 28px)" }}>Mark Attendance for Today</button>
      ) : (
        <div style={{ ...s.card, marginBottom: "clamp(20px, 3vw, 28px)", background: `${sc(attendance[today].status)}18`, border: `1px solid ${sc(attendance[today].status)}40` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><div style={{ ...s.label, marginBottom: 8, color: sc(attendance[today].status) }}>Today's Status</div><div style={{ fontSize: "clamp(16px, 2.5vw, 18px)", fontWeight: 500, color: C.white }}>{sl(attendance[today].status)} at {attendance[today].time}</div></div>
            <div style={{ ...s.badge(sc(attendance[today].status)) }}>{sl(attendance[today].status)}</div>
          </div>
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "clamp(12px, 2vw, 16px)", marginBottom: "clamp(20px, 3vw, 28px)" }}>
        {[["On Time", stats.green, C.success], ["Late", stats.yellow, C.warning], ["Very Late", stats.red, C.danger], ["Streak", streak, C.cyan]].map(([label, val, color]) => (
          <div key={label} style={{ ...s.card, borderTop: `2px solid ${color}` }}><div style={{ ...s.label, marginBottom: 8 }}>{label}</div><div style={{ fontSize: "clamp(24px, 4vw, 28px)", fontWeight: 200, color }}>{val}</div></div>
        ))}
      </div>
      <div style={s.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "clamp(16px, 2vw, 20px)" }}>
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1))} style={{ ...s.btnGhost, padding: "6px 12px" }}>Prev</button>
          <div style={{ ...s.h2 }}>{currentMonth.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</div>
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1))} style={{ ...s.btnGhost, padding: "6px 12px" }}>Next</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "clamp(4px, 1vw, 8px)", marginBottom: "clamp(12px, 2vw, 16px)" }}>
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(d => <div key={d} style={{ textAlign: "center", fontSize: "clamp(11px, 1.5vw, 12px)", fontWeight: 600, color: C.gray300 }}>{d}</div>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "clamp(4px, 1vw, 8px)" }}>
          {calendarDays.map((day, i) => {
            ...
            const bgColor = rec ? sc(rec.status) : "transparent";
            return <div key={day} style={{ aspectRatio: "1", background: rec ? `${bgColor}22` : "transparent", border: `1px solid ${isToday ? C.cyan : rec ? `${bgColor}40` : C.glassBorder}`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "clamp(12px, 1.8vw, 13px)", fontWeight: rec ? 600 : 400, color: rec ? bgColor : C.gray300 }}>{day}</div>;
          })}
        </div>
      </div>
    </div>
...
function Sidebar({ active, onNav, user, sidebarOpen, setSidebarOpen, timerActive }) {
  ...
  // Desktop sidebar is always visible, mobile is overlay
  return (
    <>
      {/* Mobile backdrop */}
      {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 99, display: "none" }} className="sidebar-backdrop" />}
      {/* Sidebar */}
      <div style={{
        position: sidebarOpen ? "fixed" : "relative",
        left: 0, top: 0,
        width: 220, minWidth: 220, height: "100vh",
        background: C.navyMid, display: "flex", flexDirection: "column",
        borderRight: `1px solid ${C.glassBorder}`, flexShrink: 0,
        zIndex: sidebarOpen ? 100 : 1,
        transform: sidebarOpen ? "translateX(0)" : undefined,
      }} className="sidebar-panel">
        <div style={{ padding: "clamp(12px, 2.5vw, 16px)", borderBottom: `1px solid ${C.glassBorder}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 6, background: `linear-gradient(135deg, ${C.cyan}, ${C.accentAlt})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, color: C.navy }}>IX</div>
            <div><div style={{ fontWeight: 700, fontSize: 13, letterSpacing: "0.1em", color: C.white }}>IITIX LABS</div><div style={{ fontSize: 9, color: C.gray300, letterSpacing: "0.1em" }}>COMMAND</div></div>
          </div>
        </div>
        ...
        <div style={{ padding: "clamp(12px, 2vw, 12px)", borderTop: `1px solid ${C.glassBorder}`, fontSize: "clamp(10px, 1.5vw, 11px)" }}>
          ...
          {user && <><div style={{ fontSize: "clamp(12px, 1.8vw, 13px)", color: C.white, fontWeight: 500, marginBottom: 2 }}>{user.name}</div><div style={{ fontSize: "clamp(10px, 1.5vw, 11px)", color: C.cyan, letterSpacing: "0.04em", marginBottom: 6 }}>{user.id}</div></>}
...
// ─── TOPBAR ───────────────────────────────────────────────────────────────
function TopBar({ page, user, onLogout, onToggleSidebar }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "clamp(12px, 2vw, 16px)", width: "100%" }}>
      <button onClick={onToggleSidebar} className="hamburger-btn" style={{ background: "transparent", border: "none", color: C.cyan, fontSize: 20, cursor: "pointer", padding: "4px 8px" }}>☰</button>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "clamp(8px, 2vw, 12px)", flexShrink: 0 }}>
        <span className="topbar-date" style={{ fontSize: "clamp(10px, 1.5vw, 12px)", color: C.gray300 }}>{new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}</span>
...
function DashboardPage({ user, profile }) {
  ...
  const focusTrend = Array.from({ length: 7 }, (_, i) => {
    ...
    const daySessions = sessions.filter(s => new Date(s.timestamp).toISOString().split("T")[0] === ds);
    return { day: d.toLocaleDateString("en", { weekday: "short" }), minutes: Math.round(daySessions.reduce((a, s) => a + s.duration, 0) / 60000) };
  });

  return (
    <div>
      <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}><h1 style={{ ...s.h1, marginBottom: 4 }}>Command Dashboard</h1><p style={{ ...s.muted }}>Your operational command center</p></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "clamp(10px, 2vw, 16px)", marginBottom: "clamp(20px, 3vw, 28px)" }}>
        {[["Mission Countdown", daysLeft, C.cyan, "days to exam"], ["Tasks Complete", `${completedTasks}/${tasks.length}`, C.success, "done / total"], ["Upcoming Tests", tests.filter(t => !t.result).length, C.warning, "pending"]].map(([label, val, color, sub]) => (
          <div key={label} style={{ ...s.card, borderTop: `2px solid ${color}` }}><div style={{ ...s.label, marginBottom: 8 }}>{label}</div><div style={{ fontSize: "clamp(24px, 4vw, 30px)", fontWeight: 200, color }}>{val}</div><div style={{ ...s.muted, fontSize: 11, marginTop: 4 }}>{sub}</div></div>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "clamp(16px, 2.5vw, 20px)" }}>
        <div style={s.card}>
          ...
          {focusTrend.every(d => d.minutes === 0) ? <div style={{ color: C.gray300, fontSize: 13, textAlign: "center", padding: 24 }}>No focus sessions yet</div> : (
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={focusTrend}><CartesianGrid strokeDasharray="3 3" stroke={C.glassBorder} /><XAxis dataKey="day" tick={{ fill: C.gray300, fontSize: 11 }} /><YAxis tick={{ fill: C.gray300, fontSize: 11 }} />
                ...
                <Line type="monotone" dataKey="minutes" stroke={C.cyan} strokeWidth={2} dot={{ fill: C.cyan, r: 3 }} /></LineChart>
            </ResponsiveContainer>
          )}
        </div>
        <div style={s.card}>
          <div style={{ ...s.h3, marginBottom: 16 }}>Profile Summary</div>
          {[["Target Exam", profile?.exam], ["Exam Year", profile?.year], ["Mode", profile?.coaching], ["Target Rank", profile?.targetRank]].map(([label, value]) => (
            <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${C.glassBorder}` }}>
              <span style={{ ...s.label }}>{label}</span><span style={{ fontSize: 14, color: C.white, fontWeight: 500 }}>{value || "—"}</span>
            </div>
          ))}
        </div>
      </div>
...
function OperationsPage({ user }) {
  ...
  const addTask = () => { if (!form.title.trim()) return; saveTasks([...tasks, { id: Date.now(), ...form, status: "pending", createdAt: Date.now() }]); setForm({ title: "", subject: "Physics", deadline: "", priority: "Medium" }); setShowForm(false); };
  ...
  const pc = p => p === "High" ? C.danger : p === "Medium" ? C.warning : C.success;
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "clamp(20px, 3vw, 28px)", flexWrap: "wrap", gap: 12 }}>
        <div><h1 style={{ ...s.h1, marginBottom: 4 }}>Operations</h1><p style={{ ...s.muted }}>Manage your study tasks</p></div>
      ...
      {showForm && (
        <div style={{ ...s.card, marginBottom: "clamp(20px, 3vw, 28px)", border: `1px solid ${C.cyan}40` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 12 }}>
            <div style={{ gridColumn: "1 / -1" }}><div style={{ ...s.label, marginBottom: 6 }}>Title</div><input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Task title..." style={{ ...s.input }} onKeyDown={e => e.key === "Enter" && addTask()} /></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Subject</div><select value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} style={{ ...s.input }}>{["Physics", "Chemistry", "Mathematics", "Biology", "Other"].map(s => <option key={s}>{s}</option>)}</select></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Priority</div><select value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value }))} style={{ ...s.input }}>{["High", "Medium", "Low"].map(p => <option key={p}>{p}</option>)}</select></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Deadline</div><input type="date" value={form.deadline} onChange={e => setForm(f => ({ ...f, deadline: e.target.value }))} style={{ ...s.input }} /></div>
          </div>
          <div style={{ display: "flex", gap: 10 }}><button onClick={addTask} style={{ ...s.btn }}>Add Task</button><button onClick={() => setShowForm(false)} style={{ ...s.btnGhost }}>Cancel</button></div>
        </div>
      )}
      {tasks.length === 0 ? <div style={{ ...s.card, textAlign: "center", padding: 40, color: C.gray300 }}>No tasks yet</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{tasks.map(t => (
          <div key={t.id} style={{ ...s.card, display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", opacity: t.status === "done" ? 0.65 : 1 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, color: C.white, fontWeight: 500, textDecoration: t.status === "done" ? "line-through" : "none", marginBottom: 4 }}>{t.title}</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><span style={{ ...s.tag(C.cyan), fontSize: 11 }}>{t.subject}</span><div style={{ width: 8, height: 8, borderRadius: "50%", background: pc(t.priority) }} /></div>
            </div>
            <button onClick={() => deleteTask(t.id)} style={{ background: "transparent", border: "none", color: C.danger, cursor: "pointer", fontSize: 14, padding: "4px 8px" }}>✕</button>
          </div>
        ))}</div>
      )}
    </div>
...
function AnalyticsPage({ user }) {
  ...
  const tasks = store.get("tasks_" + uid, []);
  const sessions = store.get("sessions", []);
  const subjectData = ["Physics", "Chemistry", "Mathematics", "Biology", "Other"].map(sub => ({ subject: sub.slice(0, 4), tasks: tasks.filter(t => t.subject === sub).length, done: tasks.filter(t => t.subject === sub && t.status === "done").length })).filter(d => d.tasks > 0);
  const focusTrendData = Array.from({ length: 14 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - (13 - i)); const ds = d.toISOString().split("T")[0]; return { day: d.toLocaleDateString("en", { weekday: "short" }), minutes: Math.round(sessions.filter(s => new Date(s.timestamp).toISOString().split("T")[0] === ds).reduce((a, s) => a + s.duration, 0) / 60000) }; });

  return (
    <div>
      <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}><h1 style={{ ...s.h1, marginBottom: 4 }}>Analytics</h1><p style={{ ...s.muted }}>Your performance insights</p></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "clamp(16px, 2.5vw, 20px)" }}>
        <div style={s.card}><div style={{ ...s.h3, marginBottom: 16 }}>Subject Coverage</div>
          {subjectData.length === 0 ? <div style={{ color: C.gray300, fontSize: 13, textAlign: "center", padding: 24 }}>No data yet</div> : (
            <ResponsiveContainer width="100%" height={200}><BarChart data={subjectData}><CartesianGrid strokeDasharray="3 3" stroke={C.glassBorder} /><XAxis dataKey="subject" tick={{ fill: C.gray300, fontSize: 11 }} /><YAxis tick={{ fill: C.gray300, fontSize: 11 }} />
              <Tooltip contentStyle={{ background: C.navyMid, border: `1px solid ${C.glassBorder}`, borderRadius: 8 }} /><Bar dataKey="tasks" fill={`${C.cyan}99`} radius={[4, 4, 0, 0]} /><Bar dataKey="done" fill={C.success} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
          )}
        </div>
        <div style={s.card}><div style={{ ...s.h3, marginBottom: 16 }}>Focus Trend — 14 Days</div>
          {focusTrendData.every(d => d.minutes === 0) ? <div style={{ color: C.gray300, fontSize: 13, textAlign: "center", padding: 24 }}>No data yet</div> : (
            <ResponsiveContainer width="100%" height={200}><LineChart data={focusTrendData}><CartesianGrid strokeDasharray="3 3" stroke={C.glassBorder} /><XAxis dataKey="day" tick={{ fill: C.gray300, fontSize: 11 }} /><YAxis tick={{ fill: C.gray300, fontSize: 11 }} />
              <Tooltip contentStyle={{ background: C.navyMid, border: `1px solid ${C.glassBorder}`, borderRadius: 8 }} formatter={v => [v + " min", "Focus"]} /><Line type="monotone" dataKey="minutes" stroke={C.cyan} strokeWidth={2} dot={{ fill: C.cyan, r: 3 }} /></LineChart></ResponsiveContainer>
          )}
        </div>
      </div>
    </div>
...
function AssessmentsPage({ user }) {
  ...
  const addTest = () => { if (!form.title.trim()) return; saveTests([...tests, { id: Date.now(), title: form.title, date: form.date, score: Number(form.score), topper: Number(form.topper), total: Number(form.total), result: !!form.score }]); setForm({ title: "", date: "", score: "", topper: "", total: "100" }); setShowForm(false); };

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "clamp(20px, 3vw, 28px)", flexWrap: "wrap", gap: 12 }}>
        <div><h1 style={{ ...s.h1, marginBottom: 4 }}>Assessments</h1><p style={{ ...s.muted }}>Track your test scores</p></div>
      ...
      {showForm && (
        <div style={{ ...s.card, marginBottom: 20, border: `1px solid ${C.cyan}40` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 12 }}>
            <div style={{ gridColumn: "1 / -1" }}><div style={{ ...s.label, marginBottom: 6 }}>Test Name</div><input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. JEE Mock #3" style={{ ...s.input }} /></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Date</div><input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} style={{ ...s.input }} /></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Total Marks</div><input type="number" value={form.total} onChange={e => setForm(f => ({ ...f, total: e.target.value }))} style={{ ...s.input }} /></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Your Score</div><input type="number" value={form.score} onChange={e => setForm(f => ({ ...f, score: e.target.value }))} style={{ ...s.input }} /></div>
            <div><div style={{ ...s.label, marginBottom: 6 }}>Topper Score</div><input type="number" value={form.topper} onChange={e => setForm(f => ({ ...f, topper: e.target.value }))} style={{ ...s.input }} /></div>
          </div>
          <div style={{ display: "flex", gap: 10 }}><button onClick={addTest} style={{ ...s.btn }}>Log Test</button><button onClick={() => setShowForm(false)} style={{ ...s.btnGhost }}>Cancel</button></div>
        </div>
      )}
      {tests.length === 0 ? <div style={{ ...s.card, textAlign: "center", padding: 40, color: C.gray300 }}>No test results yet</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{tests.slice().reverse().map(t => {
          const pct = t.total ? Math.round((t.score / t.total) * 100) : 0; const color = pct >= 70 ? C.success : pct >= 50 ? C.warning : C.danger;
          return <div key={t.id} style={{ ...s.card, display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ flex: 1 }}><div style={{ fontSize: 14, color: C.white, fontWeight: 500, marginBottom: 4 }}>{t.title}</div><div style={{ fontSize: 12, color: C.gray300 }}>{t.date}</div></div>
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 20, fontWeight: 200, color }}>{t.score}/{t.total}</div><div style={{ ...s.label, color }}>{pct}%</div></div>
            {t.topper > 0 && <div style={{ textAlign: "center" }}><div style={{ fontSize: 13, color: C.warning }}>{t.topper}</div><div style={{ ...s.label }}>Topper</div></div>}
            <button onClick={() => saveTests(tests.filter(x => x.id !== t.id))} style={{ background: "transparent", border: "none", color: C.danger, cursor: "pointer", fontSize: 14, padding: "4px 8px" }}>✕</button>
          </div>;
        })}</div>
      )}
    </div>
...
function ReportsPage({ user }) {
  ...
  const sessions = store.get("sessions", []);
  const tests = store.get("tests_" + uid, []);
  const totalFocus = Math.round(sessions.reduce((a, s) => a + s.duration, 0) / 60000);
  const avgScore = tests.length ? Math.round(tests.reduce((a, t) => a + (t.total ? (t.score / t.total) * 100 : 0), 0) / tests.length) : null;
  const weekData = Array.from({ length: 7 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - (6 - i)); const ds = d.toISOString().split("T")[0]; return { day: d.toLocaleDateString("en", { weekday: "short" }), focus: Math.round(sessions.filter(s => new Date(s.timestamp).toISOString().split("T")[0] === ds).reduce((a, s) => a + s.duration, 0) / 60000) }; });

  return (
    <div>
      <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}><h1 style={{ ...s.h1, marginBottom: 4 }}>Reports</h1><p style={{ ...s.muted }}>Weekly performance summary</p></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 20 }}>
        {[["Total Focus", `${Math.floor(totalFocus / 60)}h ${totalFocus % 60}m`, C.cyan], ["Tasks Done", `${tasks.filter(t => t.status === "done").length}/${tasks.length}`, C.success], ["Avg Score", avgScore !== null ? avgScore + "%" : "—", C.warning], ["Tests Logged", tests.length, C.accentAlt]].map(([label, val, color]) => (
          <div key={label} style={{ ...s.card, borderTop: `2px solid ${color}` }}><div style={{ ...s.label, marginBottom: 8 }}>{label}</div><div style={{ fontSize: 22, fontWeight: 200, color }}>{val}</div></div>
        ))}
      </div>
      <div style={s.card}><div style={{ ...s.h3, marginBottom: 16 }}>Focus This Week</div>
        {weekData.every(d => d.focus === 0) ? <div style={{ color: C.gray300, fontSize: 13, textAlign: "center", padding: 24 }}>No focus sessions logged</div> : (
          <ResponsiveContainer width="100%" height={220}><BarChart data={weekData}><CartesianGrid strokeDasharray="3 3" stroke={C.glassBorder} /><XAxis dataKey="day" tick={{ fill: C.gray300, fontSize: 11 }} /><YAxis tick={{ fill: C.gray300, fontSize: 11 }} />
            <Tooltip contentStyle={{ background: C.navyMid, border: `1px solid ${C.glassBorder}`, borderRadius: 8 }} formatter={v => [v + " min", "Focus"]} /><Bar dataKey="focus" fill={C.cyan} opacity={0.8} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
        )}
      </div>
    </div>
  );
...
function ProfilePage({ user, onLogout }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}><h1 style={{ ...s.h1, marginBottom: 4 }}>Profile</h1><p style={{ ...s.muted }}>Your operator profile</p></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 20 }}>
        <div style={s.card}><div style={{ ...s.h3, marginBottom: 16 }}>Identity</div>{[["Name", user?.name], ["ID", user?.id], ["Email", user?.email]].map(([label, value]) => <div key={label} style={{ marginBottom: 14 }}><div style={{ ...s.label, marginBottom: 4 }}>{label}</div><div style={{ color: label === "ID" ? C.cyan : C.white, fontSize: label === "ID" ? 13 : 15 }}>{value || "—"}</div></div>)}</div>
        <div style={s.card}><div style={{ ...s.h3, marginBottom: 16 }}>Exam Profile</div>{[["Target Exam", profile?.exam], ["Exam Year", profile?.year], ["Mode", profile?.coaching], ["Target Rank", profile?.targetRank]].map(([label, value]) => <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${C.glassBorder}` }}><span style={{ ...s.label }}>{label}</span><span style={{ fontSize: 14, color: C.white, fontWeight: 500 }}>{value || "—"}</span></div>)}</div>
      </div>
...
export default function App() {
  ...
  const handleLogin = u => {
    ...
    if (!p) { setScreen("onboard"); return; }
    const today = new Date().toISOString().split("T")[0];
    setScreen(store.get("init_date", null) === today ? "app" : "daily-init");
  };
    ...
    <div style={{ minHeight: "100vh", background: C.navy, color: C.white, fontFamily: "'Segoe UI', system-ui, -apple-system, sans-serif", display: "flex", flexDirection: "row" }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .sidebar-panel { position: fixed !important; left: 0; top: 0; height: 100vh !important; transform: translateX(0); }
        }
        @media (min-width: 769px) {
          .hamburger-btn { display: none !important; }
          .sidebar-backdrop { display: none !important; }
          .sidebar-panel { position: relative !important; }
          .topbar-date { display: inline !important; }
        }
        @media (max-width: 768px) {
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto" }}>
        <div style={{ background: C.navyMid, borderBottom: `1px solid ${C.glassBorder}`, display: "flex", alignItems: "center", padding: "clamp(12px, 3vw, 28px)", gap: 16, flexShrink: 0, zIndex: 50 }}>
          <TopBar page={page} user={user} onLogout={handleLogout} onToggleSidebar={() => setSidebarOpen(!sidebarOpen)} />
        </div>
        <div style={{ flex: 1, padding: "clamp(16px, 4vw, 28px)", overflowY: "auto" }}>
